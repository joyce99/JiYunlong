...
parser.add_argument('--l2-w', type=float, default=0.01, help='weight of L2-norm regularization')  #0.01
parser.add_argument("--num-heads", type=int, default=4, help="number of hidden attention heads")  #4
parser.add_argument("--num-layers", type=int, default=2, help="number of hidden layers")  #本来是2
parser.add_argument("--num-hidden", type=int, default=16, help="number of hidden units")  #16
...
parser.add_argument("--in-drop", type=float, default=0.4, help="input feature dropout")  #0.4
parser.add_argument("--attn-drop", type=float, default=0.1, help="attention dropout")  #0.1
...
parser.add_argument("--batch_size", type=int, default=1000, help="batch_size for each domain")
parser.add_argument("--threashold", type=float, default=0.1, help="remove nodes whose similarity less than 0.3")
parser.add_argument('--data_src', type=str, default='Cora1', help='source dataset name')  #Blog1_new  Citeseer1
parser.add_argument('--data_trg', type=str, default='Cora2', help='target dataset name')  #Blog2_new  Citeseer2
...
f.flush()
# Load source data
A_s, X_s, Y_s = load_network('./Datasets_changed/4/'+emb_filename+'/' + str(source) + '.mat')
...
# Load target data
A_t, X_t, Y_t = load_network('./Datasets_changed/4/'+emb_filename+'/' + str(target) + '.mat')
...
while random_state < numRandom:
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (batch_s, batch_t) in enumerate(
            ...
            # mean_attention_t_new = mean_attention_t.clone()
            #
            # # 获取唯一的src值
            # unique_src = torch.unique(src)
            #
            # # 对每一个唯一的src值进行处理
            # for value in unique_src:
            #     # 找到src中等于当前值的位置
            #     indices = (src == value).nonzero(as_tuple=True)[0]
            #
            #     # 获取mean_attention_t中对应的值
            #     corresponding_values = mean_attention_t[indices]
            #
            #     min_val = torch.min(corresponding_values)
            #     max_val = torch.max(corresponding_values)
            #
            #     if min_val == max_val:
            #         normalized_mean_attention_t=torch.ones_like(corresponding_values)
            #     else:
            #         # 进行0到1的标准化
            #         normalized_mean_attention_t = (corresponding_values - min_val) / (max_val - min_val)
            #
            #     mean_attention_t_new[indices] = normalized_mean_attention_t


                # mean_val = torch.mean(corresponding_values)
                # std_val = torch.std(corresponding_values)
                #
                # # 如果标准差不为0，则进行标准化，否则直接减去均值
                # if std_val != 0:
                #     standardized_values = (corresponding_values - mean_val) / std_val
                # else:
                #     standardized_values = corresponding_values - mean_val
                #
                # # 将mean_attention_t中对应值都除以这个最大值
                # mean_attention_t_new[indices] = standardized_values
            ...
            #     if label1!=label2:
            #         different+=1
            #     if (label1<label_s.shape[1] and label2>=label_s.shape[1]) or label1>=label_s.shape[1] and label2<label_s.shape[1]:
            #         kno_unk_count+=1
            ...
            # pred_logit_t_known = pred_logit_t[:, :Y_s.shape[1]]
            # entropy = -torch.sum(pred_logit_t_known * torch.log(pred_logit_t_known.clamp(min=1e-9)), dim=1)

            entropy = -torch.sum(pred_logit_t * torch.log(pred_logit_t.clamp(min=1e-9)), dim=1)  #原来的

            pseudo_unknown_num = int(entropy.shape[0] * 0.2)
            ...
            _,pseudo_known = torch.topk(entropy, (int)(entropy.shape[0]*0.3), largest=False)
            ...
            if clf_type == 'multi-class':
                ...
                # pred_logit_s1=pred_logit_s[:, :label_s.shape[1]]
                # clf=clf_loss_f(pred_logit_s, (aug_label_s).float())
                if epoch<epoch_thre:
                    clf_loss = torch.sum(clf_loss_f(pred_logit_s, (aug_label_s).float()))/label_s.shape[0]
                    print("clf_loss1: ",clf_loss.item())
                else:
                    ...
                    clf_loss = torch.sum(clf_loss_f(pred, (label).float()))/label.shape[0]
                    print("clf_loss2: ",clf_loss.item())
                 # = clf_loss / ((label_s).shape[0]+pred_logit_t1_unknown.shape[0])
            ...
            if epoch<epoch_thre:
                ...
                total_loss = domain_loss + clf_loss + center_contrastive_loss + node_contrastive_loss#  #
            # optimizer.zero_grad()
            total_loss.backward()
        ...
        with torch.no_grad():
            ...
            if HOS>best_hos:
                ...
                best_hos=HOS
                best_epoch = epoch
                scipy.io.savemat('./' + emb_filename + '_emb_three.mat',
